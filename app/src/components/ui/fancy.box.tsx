...
export function FancyBox({ fields }: FancyBoxProps) {
  ...
  const toggleFramework = (framework: Framework) => {
    const isAlreadySelected = selectedValues.some((f) => f.value === framework.value);
    if (!isAlreadySelected) {
      framework.color = generateRandomColor(); // Assign random color
      setSelectedValues((prev) => [...prev, framework]);
    } else {
      setSelectedValues((prev) => prev.filter((l) => l.value !== framework.value));
    }
  ...
  const selectables = fields.filter((field) => !selectedValues.includes(field));
    ...
    <div className="flex ">
      <div className="flex-1">
        <Popover open={openCombobox} onOpenChange={onComboboxOpenChange}>
          ...
          <PopoverContent className="w-[350px] max-h-[500px]  overflow-y- p-0">
            <Command loop>
              <CommandInput
                ref={inputRef}
              ...
              <CommandGroup className="overflow-y-auto p-0">
                {fields.map((field) => {
                  const isActive = selectedValues.some((f) => f.value === field.value);
                  return (
                    <CommandItem
                      key={field.value}
                      value={field.value}
