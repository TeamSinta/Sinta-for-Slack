...
export async function getSlackIdsOfGreenHouseUsers(
  ...
  hiring_room_recipient.recipients.forEach((recipient: { source: string; value: string | string[]; }) => {
      if (recipient.source == "greenhouse") {
...
export async function inviteUsersToChannel(channelId: any, userIds: any[], slackTeamId: string) {
  try {
