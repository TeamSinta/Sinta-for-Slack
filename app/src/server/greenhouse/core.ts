...
export async function fetchActiveCandidates() {
  ...
  return candidates.filter(candidate => candidate.applications.some(app => app.status === 'active'))
                   .map(candidate => {
                       const activeApplications = candidate.applications.filter(app => app.status === 'active');
                       return {
                           id: candidate.id,
                           name: `${candidate.first_name} ${candidate.last_name}`,
                           stage: activeApplications[0]?.current_stage?.name || 'N/A',
                           job: activeApplications.map(app => app.jobs.map(job => job.name)).flat().join(', ')
                       };
                   });
}
