...
export async function updateWorkflowStatusMutation(props: UpdateWorkflowStatusProps) {
  try {
      const workflowParse = await workflowStatusUpdateSchema.safeParseAsync(props);
      if (!workflowParse.success) {
          throw new Error("Invalid workflow data", {
              cause: workflowParse.error.errors,
          });
...
export async function updateWorkflowNameMutation(props: UpdateWorkflowNameProps) {
  try {
      const workflowParse = await workflowNameUpdateSchema.safeParseAsync(props);
      if (!workflowParse.success) {
          throw new Error("Invalid workflow data", {
              cause: workflowParse.error.errors,
          });
