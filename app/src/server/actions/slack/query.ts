...
export async function getAccessToken(teamId: string): Promise<string> {
  ...
  if (!organization.slack_access_token || organization.token_expiry === null || Date.now() >= (organization.token_expiry ?? 0) * 1000) {
      return await refreshTokenIfNeeded(teamId, organization.token_expiry!, organization.slack_refresh_token!); // This will refresh the token if necessary
  }
...
export async function refreshTokenIfNeeded(teamId: string, token_expiry: number, slack_refresh_token:string): Promise<string> {
