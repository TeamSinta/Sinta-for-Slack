    ...
    workflow.messageFields.forEach((field) => {
      switch (field) {
        ...
        case "Recruiter":
          ...
          break;
        default:
          const fieldData = interview[field as keyof ScheduledInterview];
...
interface ScheduledInterview {
  ...
  organizer: { id: number; first_name: string; last_name: string; name: string; employee_id: string };
  interviewers: Array<{ id: number; employee_id: string; name: string; email: string; response_status: string; scorecard_id: number }>;
}

interface WorkflowRecipient {
  recipients: Array<{ label: string; value: string; source: string; slackValue?: string }>;
  openingText: string;
  messageFields: Array<string>;
  messageButtons: Array<{ type: string; label: string; action: string; linkType: string }>;
