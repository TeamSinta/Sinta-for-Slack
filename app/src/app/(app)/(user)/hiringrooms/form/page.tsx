...
function HiringroomFormPage() {
  ...
  const handleCompleteTab = (tab: keyof typeof completedTabs) => {
      setCompletedTabs({ ...completedTabs, [tab]: true });
  };
...
<motion.div
  ...
  className="bg-slate-50 min-h-screen min-w-[74vw] relative"
>
  {/* Sidebar Border */}
  <div className="absolute left-0 top-0 h-full w-[2px] bg-gray-300"></div>

  {/* Main Content */}
  <div className="p-6 mb-6 ml-[1px]  overflow-y-scroll flex justify-between">
    {/* Left Side: Cards and Form */}
    <div className="w-2/3">
      {/* Header */}
      <header className="pb-4">
        <div className="flex items-center gap-2 text-3xl font-heading font-semibold">
          <span>Hire Room</span>
          <span className="text-gray-400"> &gt; </span>
          <span className="text-gray-400">New Room</span>
          <span className="ml-2 inline-block px-2 py-1 text-sm font-medium bg-yellow-100  rounded">
            Draft
          </span>
        </div>
      </header>
      ...
      <Tabs defaultValue="trigger" className="space-y-4">
        ...
        <TabsList className="flex gap-4 mt-2 w-1/2 bg-slate-200 shadow rounded-sm">
          <TabsTrigger value="trigger" className="flex w-full items-center gap-2 rounded-sm">
            <CheckCircle className={`h-5 w-5 ${completedTabs.trigger ? 'text-green-500' : 'text-gray-400'}`} />
            Setup
          </TabsTrigger>
          <TabsTrigger value="slack" className="flex w-full items-center gap-2 rounded-sm">
            <CheckCircle className={`h-5 w-5 ${completedTabs.slack ? 'text-green-500' : 'text-gray-400'}`} />
            Customization
          </TabsTrigger>
        </TabsList>
        <div className="mb-4 my border-t border-gray-300"></div>

        {/* Remaining Content (Forms, Conditions, etc.) */}
        <TabsContent value="trigger">
          <form id="hiring-room-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">

            {/* General Settings Section */}
            <Card className="rounded-sm">
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-gray-700">General</CardTitle>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    className="mt-1 w-full rounded-sm border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 text-base h-12"
                  />
                </div>

                <div>
                  <Label htmlFor="objectField" className="text-sm font-medium text-gray-700">Select Slack Channel Type</Label>
                  <Select
                    value={selectedValue}
                    onValueChange={(value) => {
                  ...
                  >
                    <SelectTrigger className="mt-1 w-full rounded-sm border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 text-base h-12">
            ...
            <Card className="rounded-sm">
          <CardHeader>
            <CardTitle className="text-xl font-semibold text-gray-800">Conditions</CardTitle>
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="space-y-4">
                  <div className="flex gap-6">
                    <div className="w-1/2">
                      <Label className="text-sm font-medium text-gray-700">Date Property</Label>
                      <Select
                        onValueChange={(value) =>
        ...
        <TabsContent value="slack">
          <form id="hiring-room-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Card className="rounded-sm">
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-gray-700">Slack Channel Format</CardTitle>
                <CardDescription className="mt-1 text-sm text-gray-600">
    ...
    <div className="w-1/3 pl-8 mt-32">
      <div className="bg-white rounded-sm shadow-md p-4">
        <h3 className="text-lg font-semibold text-gray-700">Helpful Tips</h3>
        <p className="mt-2 text-sm text-gray-600">
        Hiring Rooms are Slack channels dedicated to managing the hiring process. By configuring the right team and automating actions, you can streamline communication and collaboration.
      </p>
      <ul className="list-disc list-inside mt-4 text-sm text-gray-600">
        <li>Add key team members like recruiters, interviewers, and hiring managers.</li>
        <li>Set up automated notifications for candidate updates and interview reminders.</li>
        <li>Define specific triggers to auto-archive channels based on candidate stages.</li>
        <li>Customize workflows to notify the right people at the right time.</li>
      </ul>
...
<motion.div
  ...
  className="fixed bottom-0 right-0 mb-4 mr-4 z-50"  // Ensure it's fixed at the bottom with a higher z-index
>
  <motion.div
    whileHover={{ scale: 1.05, transition: { duration: 0.3 } }}  // Slight scale effect on hover
    className="bg-gray-900 text-white rounded p-3 shadow-xl hover:bg-gray-800 transition-all ease-in-out"
  >
    <div className="flex gap-2 items-center px-4">
      <p className="text-sm">Unsaved changes</p>
      <Button variant="outline" className="text-white  bg-gray-500  border-gray-500 hover:bg-gray-400  rounded">
        Cancel
      </Button>
      <Button variant="secondary" className="text-black hover:bg-gray-400 bg-white transition-colors rounded">
        Save Draft
      </Button>
      <Button className="bg-indigo-500 text-white px-4 py-2  rounded-md shadow-lg hover:bg-indigo-600 transition-colors rounded">
        Publish
      </Button>
    </div>
  </motion.div>
