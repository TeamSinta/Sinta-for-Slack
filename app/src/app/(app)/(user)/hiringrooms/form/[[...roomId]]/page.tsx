...
import CreateHiringRoom from "../_components/createHiringRoom";
import EditHireRoom from "../_components/EditHireRoom";

export default async function FormPageRooms({
    params,
}: {
    ...
    const activeWehooks = (await getOrganizationWebhooks()) ?? [];
    return (
        <>
            {/* Check if roomId is "new" and render CreateHiringRoom */}
            {roomId?.includes("new") ? (
                <CreateHiringRoom activeWebhooks={activeWehooks} />
            ) : roomId ? (
