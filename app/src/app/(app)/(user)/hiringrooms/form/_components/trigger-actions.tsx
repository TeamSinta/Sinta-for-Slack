...
const TriggerActionsComponent = () => {
  const [selectedAction, setSelectedAction] = useState<string>(''); // Tracks the current selected action
  const [showDropdown, setShowDropdown] = useState<boolean>(false); // Controls dropdown visibility
  const [selectedRecipients, setSelectedRecipients] = useState<{ label: string; value: string }[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        const combinedOptions = [
          ...usersData.map((user) => ({ label: user.name, value: user.id, source: 'slack' })),
          ...
          { label: greenhouseData.interviewer, value: greenhouseData.interviewer, source: 'greenhouse' },
        ];
  ...
  const handleRecipientsChange = (updatedRecipients: { label: string; value: string }[]) => {
  ...
  const renderActionComponent = (action: string) => {
    switch (action) {
      case 'add_recipients':
        return (
          <div key={action} className="mt-4 border rounded-md p-4 space-y-4">
            <Label className="text-sm font-semibold">Add Recipients</Label>
            <FancyMultiSelect
            ...
            <div className="mt-4">
              ...
              <ConditionComponent
                condition={conditions[0] || { field: '', operator: '', value: '', fieldLabel: '', operatorLabel: '', valueLabel: '' }} // Ensure a condition object exists
                index={0}
      ...
      case 'remove_recipients':
        return (
          <div key={action} className="mt-4 border rounded-md p-4 space-y-4">
            <Label className="text-sm font-semibold">Remove Recipients</Label>
            <FancyMultiSelect
              selectedOptions={selectedRecipients}
              onOptionChange={handleRecipientsChange}
              options={options} // Use fetched options
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-xl font-semibold text-gray-800">Automated Actions</CardTitle>
        <CardDescription className="mt-2 text-sm text-gray-600">
          Set up automated actions for this hiring room based on the candidate's stage.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {showDropdown && (
          <div className="mt-4">
            ...
            >
              <SelectTrigger className="w-full border-gray-300 rounded-lg shadow-sm mt-2">
