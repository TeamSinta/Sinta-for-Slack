...
const GreenhouseLogo = () => <img src={greenhouselogo.src} alt="Greenhouse Logo" className="h-4 w-4 mr-2 inline-block" />
...
export function ConditionSelector({ attributes, onFieldSelect, ...props }: ConditionSelectorProps) {
  const [open, setOpen] = React.useState(false);  // Popover open state
  const [selectedAttribute, setSelectedAttribute] = React.useState(attributes[0]);  // Selected attribute state
  const [hoveredAttribute, setHoveredAttribute] = React.useState<string | null>(null);  // Hovered attribute state
          ...
          <CommandList className="h-[var(--cmdk-list-height)] max-h-[400px]">
            ...
            <CommandGroup heading="Conditions">
              {attributes.map((attribute) => (
                <div
                  key={attribute.field}
                  onMouseEnter={() => setHoveredAttribute(attribute.field)} // Set hovered attribute
                  onMouseLeave={() => setHoveredAttribute(null)} // Reset hovered attribute
                >
                  <HoverCard open={hoveredAttribute === attribute.field}> {/* Open only if hovered */}
                    <HoverCardTrigger asChild>
                      <div>
                        <AttributeItem
                          attribute={attribute}
                          isSelected={selectedAttribute?.field === attribute.field}
                    ...
                    >
                      <div className="grid gap-2">
                        <h4 className="font-medium leading-none">{attribute.field}</h4>
...
function AttributeItem({ attribute, isSelected, onSelect }: AttributeItemProps) {
  return (
    <CommandItem
      key={attribute.field}
      onSelect={onSelect}
