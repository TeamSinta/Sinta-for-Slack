...
function CreateWorkflowSheet() {
  ...
  useEffect(() => {
    if (selectedAlertType === "timebased") {
        ...
        setConditions([
            {
                field: { value: "when stuck-in-stage in", label: "When Stuck in Stage" },
  ...
  const handleSelectChange = (value: string, label: string, field: "objectField" | "alertType" | "organizationId" | "jobId" | "stage") => {
      switch (field) {
          case "objectField":
          ...
          case "jobId":
              ...
              break;
          case "stage":
              const conditionIndex = conditions.findIndex((condition) => condition.field.value === "when stuck-in-stage in");
              handleConditionChange(conditionIndex, "field", { value: value, label });
              break;
          default:
              break;
      }
            ...
            <Select
                onValueChange={(value) =>
                    handleConditionChange(
                        0,
                        "field",
                        { value, label: dateFieldOptions.find(opt => opt.value === value)?.label || value },
                    )
                }
            >
