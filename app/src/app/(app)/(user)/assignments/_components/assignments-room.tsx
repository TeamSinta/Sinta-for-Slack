...
export function AssignmentsRoom({ assignmentsPromise }: any) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const initialRecruiterCounts =
                getRecruiterCounts(greenhouseCandidates);
            const initialCoordinatorCounts =
                getCoordinatorCounts(greenhouseCandidates);
            ...
            const greenhouseCandidateDict = greenhouseCandidates.reduce(
                (acc, cand) => {
                    acc[cand.id] = cand;
                    return acc;
                },
            ...
            sortedCandidates.forEach((cand, i) => {
                ...
                const curJobHiringManagers =
                    curJobHiringTeam.hiring_managers.map((user) => ({
                        ...user,
                        role: "Hiring Manager",
                    }));
                const curJobCoordinators = curJobHiringTeam.coordinators.map(
                    (user) => ({ ...user, role: "Coordinator" }),
                );
                const curJobSourcers = curJobHiringTeam.sourcers.map(
                    (user) => ({ ...user, role: "Sourcer" }),
                );
                const curJobRecruiters = curJobHiringTeam.recruiters.map(
                    (user) => ({ ...user, role: "Recruiter" }),
                );

                const combinedUsers = [
                    ...curJobHiringManagers,
                    ...curJobCoordinators,
                    ...curJobSourcers,
                    ...curJobRecruiters,
                ];
            ...
            const slackChannelsDict = slackChannelsCreated.reduce(
                (acc, channel) => {
                    if (channel.isArchived) {
                        return acc;
                    } else {
                        if (!acc[channel.greenhouseCandidateId])
                            acc[channel.greenhouseCandidateId] = {};
                        acc[channel.greenhouseCandidateId].channelId =
                            channel.channelId;
                        acc[channel.greenhouseCandidateId].channelName =
                            `${channel.name}`;
                        acc[channel.greenhouseCandidateId].channelLink =
                            `https://slack.com/app_redirect?channel=${channel.channelId}`;
            ...
            const filteredCandidates = greenhouseCandidates.filter(
                (candidate) => {
                    const applicationDate = new Date(
                        candidate.applications[0].applied_at,
                    );
                    return (
    ...
    function getSlackUsersFromHiringTeam(curJobHiringTeam) {
        ...
        const combinedUsers = curJobHiringManagers
            .concat(curJobCoordinators)
            .concat(curJobSourcers)
            .concat(curJobRecruiters);
    ...
    const createSlackChannelForCandidate = async (candidate) => {
        ...
        const recInitials = candidate.recruiter
            ? candidate.recruiter.first_name.substring(0, 1).toLowerCase() +
              candidate.recruiter.last_name.substring(0, 1).toLowerCase()
            : "";
        const coordInitials = candidate.coordinator
            ? candidate.coordinator.first_name.substring(0, 1).toLowerCase() +
              candidate.coordinator.last_name.substring(0, 1).toLowerCase()
            : "";
        const candName =
        ...
        <div className="overflow-x-auto">
            <div className="flex flex-col">
                ...
                <div className="flex flex-col">
                    ...
                    >
                        {showCounts
                            ? "Hide Counts Recruiter / Coordinator Candidate Count"
                            : "Show Recruiter / Coordinator Candidate Count"}
                    </button>
                </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 overflow-hidden rounded-lg border border-gray-200 dark:divide-gray-700 dark:border-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800">
                    {candidatesToAssign.map((candidate) => (
                        <tr
                            ...
                            key={candidate.id}
                        >
                            <td className="whitespace-nowrap px-3 py-1 text-sm font-medium text-gray-900 dark:text-gray-100">
                                ...
                                <div className="flex flex-col space-y-2">
                                    <div className="flex items-center">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => {
                                                const newRecruiterId =
                                                    e.target.value;
                                                handleRecruiterChange(
                                                    candidate.id,
                                                    newRecruiterId,
                                                );
                                                updateGreenhouseCandidate(
                                                    candidate,
                                                    "recruiter",
                                                    newRecruiterId,
                                                );
                                            }}
                                            className="w-100 form-select min-w-40 rounded-md border border-gray-300 px-2 py-1 dark:border-gray-600"
                                        >
                            ...
                            <td
                                className={
                                    "whitespace-nowrap px-3 py-1 text-sm font-medium text-gray-900 dark:text-gray-100 "
                                }
                            >
                                {candidate.curJobHiringTeamCombinedUsers.map(
                                    (htm, i) => (
                                        <div key={i}>
                                            <div>
                                                <div
                            ...
                            <td className="whitespace-nowrap px-3 py-1 text-sm font-medium text-gray-900 dark:text-gray-100">
                                {slackChannelsCreatedDict[candidate.id] ? (
                                    <div
                                        onClick={() => {
                                            handleArchiveSlackChannel(
                                                candidate,
                                            );
                                        }}
                                        className="button w-fit rounded bg-red-500 px-4 py-2 font-bold text-white hover:bg-red-700"
                                    >
                                        Delete -{" "}
                                        {
                                            slackChannelsCreatedDict[
                                                candidate.id
                                            ].channelName
                                        }
                                    </div>
                                    ...
                                    <div
                                        onClick={() => {
                                            createSlackChannelForCandidate(
                                                candidate,
                                            );
                                        }}
                                        className="button w-fit rounded bg-blue-500 px-4 py-2 font-bold text-white hover:bg-blue-700"
                                    >
                                        Create Channel
                                    </div>
                                )}
            ...
            {Object.keys(candidateDict).length > 0 ? (
                <AssignmentsChannelTable
                    slackChannelsCreatedPromise={assignmentsPromise}
                    greenhouseCandidateDict={candidateDict}
                    greenhouseJobsDict={jobsDict}
                ></AssignmentsChannelTable>
            ) : (
                <>LOADING...</>
            )}
                                ...
                                {!channel?.is_archived ? (
                                    <>
                                        <div
                                            ...
                                            className="button ml-8 w-fit rounded bg-red-500 px-4 py-2 font-bold text-white hover:bg-red-700"
                                        >
                                            delete Channel
                                        </div>
                                    </>
                                    ...
                                    <>
                                        <div className="font-large ml-3 border-solid font-bold">
                                            Archived
                                        </div>
                                    </>
                                )}
