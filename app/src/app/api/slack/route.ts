...
async function handleJsonPost(data:JSON) {
  ...
  return new NextResponse(JSON.stringify({ message: 'JSON POST handled successfully' }), {
      status: 200,
...
async function handleSlackInteraction(payload: SlackInteraction) {
  ...
  return new NextResponse(JSON.stringify({ error: 'Unhandled interaction type' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' }
  });
}
...
async function openModal(modalPayload: any, accessToken: string | null | undefined) {
...
async function postMessage(userId: string, accessToken: string | null | undefined) {
...
export async function POST(request: Request) {
  ...
  if (contentType?.includes('application/json')) {
      ...
      if (payloadRaw) {
          ...
          return new NextResponse(JSON.stringify({ error: 'Unrecognized form-urlencoded request' }), {
              status: 400,
              headers: { 'Content-Type': 'application/json' }
          });
      }
