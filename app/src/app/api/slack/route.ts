...
import { fetchCandidateDetails, fetchGreenhouseUsers, fetchRejectReasons, fetchStagesForJob, matchSlackToGreenhouseUsers, moveToNextStageInGreenhouse } from "@/server/greenhouse/core";
...
async function updateSlackMessage(responseUrl: string, blocks: any, attachments: any) {
  try {
    if (!responseUrl) {
...
async function handleMoveToNextStageSubmission(payload: SlackInteraction) {
  try {
    ...
    const { response_url, message_blocks, attachments, candidate_id } = JSON.parse(view.private_metadata);

    const selectedStageId = view.state.values.stage_select_block.stage_select.selected_option.value;
    ...
    const userMapping = await matchSlackToGreenhouseUsers(greenhouseUsers, slackUsers);
    ...
    let emoji = "✅";
    if (!greenhouseUserId) {
      statusMessage = "Failed to find corresponding Greenhouse user for the Slack user. This has been submitted.";
      emoji = "❌";
    } else {
      const result = await moveToNextStageInGreenhouse(candidate_id, selectedStageId, greenhouseUserId);
      if (result.success) {
        statusMessage = "Candidate moved to the next stage successfully.";
      } else {
    ...
    const actionBlockIndex = attachments[0].blocks.findIndex((block: any) => block.type === 'actions');
...
async function handleSlackInteraction(payload: SlackInteraction) {
  ...
  const { type, actions, trigger_id, team, user, response_url, message } = payload;
  ...
  if (type === "block_actions") {
    ...
    if (action_id.startsWith("move_to_next_stage_")) {
      ...
      const modalPayload = await createMoveToNextStageModal(trigger_id, candidateId, privateMetadata);
      ...
      const modalPayload = await createRejectCandidateModal(trigger_id, candidateId, privateMetadata);
      return openModal(modalPayload, accessToken);
    }
...
async function createMoveToNextStageModal(trigger_id: string, candidateId: string, privateMetadata: string) {
  try {
    ...
    let jobId;
    if (candidateDetails.applications && candidateDetails.applications.length > 0) {
...
async function createRejectCandidateModal(trigger_id: string, candidateId: string, privateMetadata: string) {
  try {
...
async function handleRejectCandidateSubmission(payload: SlackInteraction) {
  try {
    ...
    const rejectReasonId = view.state.values.reject_reason_select_block.reject_reason_select.selected_option.value;
    const emailTemplateId = view.state.values.email_template_select_block.email_template_select.selected_option.value;
    const rejectComments = view.state.values.reject_comments.reject_comments_input.value;
    ...
    if (!greenhouseUserId) {
      ...
      const result = await rejectCandidateInGreenhouse(candidate_id, greenhouseUserId, rejectReasonId, emailTemplateId, rejectComments);
      if (result.success) {
        statusMessage = "Candidate has been rejected successfully. This has been submitted.";
      } else {
