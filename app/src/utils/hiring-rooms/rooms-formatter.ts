...
export function getSlackUsersFromRecipient(recipientObject: { recipients: any[] }) {
  const slackUsers: any[] = [];

  // Check if recipients is defined and is an array
  if (!recipientObject.recipients || !Array.isArray(recipientObject.recipients)) {
    console.error("Recipients is not an array or is undefined:", recipientObject.recipients);
